...
```console
helm install storage-class oci://ghcr.io/younsl/charts/storage-class --version 0.4.0
```
...
```console
helm pull oci://ghcr.io/younsl/charts/storage-class --untar --version 0.4.0
...
| storageClasses.gp3.volumeTags | object | {} | Volume tags to apply to provisioned EBS volumes (AWS EBS CSI driver only) Tags are added as tagSpecification_<key> parameters |
...
| storageClasses.gp3-encrypted.volumeTags | object | {} | Volume tags to apply to provisioned EBS volumes (AWS EBS CSI driver only) Tags are added as tagSpecification_<key> parameters |
| volumeAttributeClasses | object | See [Volume Attributes Classes](https://kubernetes.io/docs/concepts/storage/volume-attributes-classes/) | Map of VolumeAttributeClass configurations to create Requires EKS 1.31.0+ or Vanilla Kubernetes 1.34.0+ (stable feature) |
